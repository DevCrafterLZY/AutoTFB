...
from ts_benchmark.common.constant import CHECKPOINT_PATH
...
class TransformerAdapter(ModelBase):
    ...
    def forecast_fit(
        ...
        if self.config.check_point_load_path is not None:
            check_point_load_path = os.path.join(CHECKPOINT_PATH, self.config.check_point_load_path)
            state_dict = torch.load(check_point_load_path)
            self.model.load_state_dict(state_dict)
            logger.info(f"Load {self.model_name} from {check_point_load_path}")
            return self.model
        ...
        for epoch in range(config.num_epochs):
            ...
            for i, (input, target, input_mark, target_mark) in enumerate(
                ...
                input, target, input_mark, target_mark = (
                    ...
                    input_mark.float().to(device),
                    target_mark.float().to(device),
                )
                # decoder input
                dec_input = torch.zeros_like(target[:, -config.horizon:, :]).float()
                dec_input = (
        ...
        if self.config.check_point_path is not None and self.early_stopping.check_point is not None:
            check_point_path = os.path.join(CHECKPOINT_PATH, self.config.check_point_path)
            folder = os.path.dirname(check_point_path)
            os.makedirs(folder, exist_ok=True)
            torch.save(self.early_stopping.check_point, check_point_path)
